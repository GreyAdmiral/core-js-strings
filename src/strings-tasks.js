 ...
 */
function removeFirstOccurrences(v, sub) {
  const index = v.indexOf(sub);

  if (index === -1) return v;
  return v.slice(0, index).concat(v.slice(index + sub.length));
}
 ...
 */
function removeLastOccurrences(v, sub) {
  const index = v.lastIndexOf(sub);
 ...
 *   sumOfCodes() => 0
 */
function sumOfCodes(v) {
  let res = 0;

  if (v && v.length) {
    for (let i = 0, j = v.length - 1; i <= j; i += 1) {
      res += v.charCodeAt(i);
    }
  }
 ...
 */
function startsWith(v, sub) {
  return v.startsWith(sub);
}
 ...
 */
function endsWith(v, sub) {
  return v.endsWith(sub);
}
...
function formatTime(m, s) {
  const min = m < 10 ? `${m}`.padStart(2, '0') : m;
  const sec = s < 10 ? `${s}`.padStart(2, '0') : s;
 ...
 */
function orderAlphabetically(v) {
 ...
 */
function containsSubstring(v, sub) {
...
function countVowels(v) {
  ...
  if (v.length) {
    count = v.split('').filter((it) => arr.includes(it)).length;
  }
...
function isPalindrome(v) {
  return (
    ...
    v.replace(/\W/gi, '').toLowerCase().split('').reverse().join('')
  );
}
...
function invertCase(v) {
  ...
  return `${v}`
    .split('')
    .map((it) =>
      it.toLowerCase() === it ? it.toUpperCase() : it.toLowerCase()
    )
    .join('');
}
 ...
 *
 */
function encodeToRot13(v) {
  const arr1 = 'ABCDEFGHIJKLMabcdefghijklm';
  const arr2 = 'NOPQRSTUVWXYZnopqrstuvwxyz';

  return v
    .split('')
    .map((it) => {
      if (arr1.includes(it)) {
        return arr2[arr1.indexOf(it)];
      }
      if (arr2.includes(it)) {
        return arr1[arr2.indexOf(it)];
      }
      return it;
    })
    .join('');
